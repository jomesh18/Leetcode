...
class Solution:
    def minimumCost(self, source: str, target: str, original: List[str], changed: List[str], cost: List[int]) -> int:
        ...
        for o, c, cos in zip(original, changed, cost):
            adj[ord(o)-ord('a')].append((ord(c)-ord('a'), cos))
        
        distances = [[float('inf')]*26 for _ in range(26)]
        def dijkstra(start, distance):
            ...
            while heap:
                ...
                for nei, cost in adj[curr_node]:
                    if cost + curr_cost < distance[nei]:
                        distance[nei] = cost + curr_cost
        ...
        for i, c in enumerate(source):
            if c != target[i]:
                curr_cost = distances[ord(c)-ord('a')][ord(target[i])-ord('a')]
