...
class Router:
    ...
    def forwardPacket(self) -> List[int]:
        ...
        tup = self.router.popleft()
        self.packets.remove(tup)
        self.destination[tup[2]].popleft()
        return [tup[1], tup[2], tup[0]]

    def getCount(self, destination: int, startTime: int, endTime: int) -> int:
        if destination not in self.destination:
            return 0
        l = bisect_left(self.destination[destination], startTime)
        r = bisect_right(self.destination[destination], endTime)
        return r-l
