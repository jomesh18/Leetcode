...
class Solution:
    def getLengthOfOptimalCompression(self, s: str, k: int) -> int:
        ...
        def counter(start, last, last_count, left):
            ...
            if start == len(s):
                return 0
            if (start, last, last_count, left) in memo: return memo[(start, last, last_count, left)]
            if s[start] == last:
                incr = 1 if last_count == 1 or last_count == 9 or last_count == 99 else 0
                ans = incr + counter(start+1, last, last_count+1, left)
            else:
                keep = 1 + counter(start+1, s[start], 1, left)
                delete = counter(start+1, last, last_count, left-1)
                ans = min(keep, delete)
            memo[(start, last, last_count, left)] = ans
            return ans
