...
class SegmentTree:
	...
	def query(self, pos):
		return self.query_util(pos, 0, self.n-1, 0)

	def query_util(self, v, tl, tr, pos):
		if tl == tr:
			return self.t[v]
		tm = (tl + tr) // 2
		if pos <= tm:
			return t[v] + self.query_util(2*v, tl, tm, pos)
		return t[v] + self.query_util(2*v+1, tm+1, tr, pos)
