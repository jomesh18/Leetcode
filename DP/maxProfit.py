...
#         l = len(prices)
#         def helper(i, left, buy, prev=None):
#             if left == 0 or i == l: return 0
#             if buy:
#                 return max(-prices[i]+helper(i+1, left, False, i), helper(i+1, left, True))
#             else:
#                 score = 0
#                 if prices[i] > prices[prev]:
#                     score = prices[i]+helper(i+1, left-1, True)
#                 score = max(score, helper(i+1, left, False, prev))
#                 return score
...
#     def maxProfit(self, k: int, prices: [int]) -> int:
#         l = len(prices)
#         dp = [[[float("-inf")]*2 for _ in range(k+1)] for _ in range(l)]
#         print(dp)
#         def helper(i, left, buy):
#             if left == 0 or i == l: return 0
#             print(i, left, buy)
#             if dp[i][left][buy] != float("-inf"): return dp[i][left][buy]
#             if buy:
#                 dp[i][left][buy] = max(-prices[i]+helper(i+1, left, 0), helper(i+1, left, 1))
#             else:
#                 dp[i][left][buy] = max(prices[i]+helper(i+1, left-1, 1), helper(i+1, left, 0))
